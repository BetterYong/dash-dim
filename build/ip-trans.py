...
h2.cmd("iptables -t mangle -A PREROUTING -p tcp -d 10.0.3.2  -j TPROXY --tproxy-mark 0x1/0x1 --on-port 2233")
...
h3.cmd("iptables -t mangle -N DIVERT")
h3.cmd("iptables -t mangle -A PREROUTING -p tcp -m socket -j DIVERT")
h3.cmd("iptables -t mangle -A DIVERT -j MARK --set-mark 1")
h3.cmd("iptables -t mangle -A DIVERT -j ACCEPTT")
h3.cmd("ip rule add fwmark 1 lookup 100")
h3.cmd("ip route add local 0.0.0.0/0 dev lo table 100")
h3.cmd("iptables -t mangle -A PREROUTING -p tcp -d 10.0.3.2  -j TPROXY --tproxy-mark 0x1/0x1 --on-port 2233")
