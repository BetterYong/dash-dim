...
#include "quic_stream_sequencer.h"
...
#include "quic_error_codes.h"
#include "quic_constants.h"
#include "quic_stream_sequencer_buffer.h"
...
void QuicStreamSequencer::OnFrameData(QuicStreamOffset byte_offset,
  ...
  QuicErrorCode result = buffered_frames_.OnStreamData(
      byte_offset, nonstd::string_view(data_buffer, data_len), &bytes_written,
      &error_details);
  if (result != QUIC_NO_ERROR) {
    std::string details =
        basic::StrCat("Stream ", stream_->id(), ": ",
...
size_t QuicStreamSequencer::Readv(const struct iovec* iov, size_t iov_len) {
  ...
  if (read_error != QUIC_NO_ERROR) {
    std::string details =
        basic::StrCat("Stream ", stream_->id(), ": ", error_details);
...
const std::string QuicStreamSequencer::DebugString() const {
  // clang-format off
  return basic::StrCat("QuicStreamSequencer:",
