...
void TcpConnectionLeft::ParseMessage(uint64_t offset,const char *data,size_t size){
    uint8_t type=0;
    ...
    if(success&&PROXY_MESSAGE_META==type){
        ...
        std::cout<<"ParseMessage orgin dst "<<socket_addr.ToString()<<std::endl;
        if(0==uuid_){
            uuid_=uuid;
